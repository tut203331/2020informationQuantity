package s4.B203331; // Please modify to s4.Bnnnnnn, where nnnnnn is your student ID. 
import java.lang.*;
...
/*
	private void insert(int a){
		suffixArray[num++]=a;
		int i=num,j=i/2;
		while(i>1 && suffixArray[i-1] < suffixArray[j-1]){
			int t = suffixArray[i-1];
			suffixArray[i-1] = suffixArray[j-1];
			suffixArray[j-1] = t;
			i = j;
			j = i/2;
		}
...
*/
	...
	private int deletemin(){
		...
		int i=1,j=i*2;
		while(j<=num){
			if(j+1 <= num && suffixArray[j-1] > suffixArray[j]) j++;
			if(suffixArray[i-1] > suffixArray[j-1]){
				int t = suffixArray[i-1];
				suffixArray[i-1] = suffixArray[j-1];
				suffixArray[j-1] = t;
			}
